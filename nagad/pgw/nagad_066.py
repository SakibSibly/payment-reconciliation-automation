...
def run(playwright: Playwright) -> None:
    username = config("NAGAD_USER_04")
    password = config("NAGAD_PASS_04")
    ...
    # Keep the same click sequence as reference, with dynamic day and 14 fallback.
    ...
    output_file = output_dir / f"{config('NAGAD_NUMBER_04')}_nagad_pgw_{target_date_file}.xlsx"
    download.save_as(str(output_file))
    page1.close()

    print(f"Saved Nagad PGW 066 history: {output_file}")
...
def run_nagad_066():
    print("🚀 Starting Nagad 066 Automation...")
    with sync_playwright() as playwright:
        run(playwright)
    print("✅ Nagad 066 Automation Completed!")
